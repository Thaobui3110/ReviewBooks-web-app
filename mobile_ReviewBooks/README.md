...
Thanh tab dưới cùng (`CustomTabBar.tsx`) tự vẽ hoàn toàn — không dùng thanh tab dựng sẵn của React Navigation, không icon, chỉ hiện **chữ**: nền tối, mỗi tab là 1 ô bo góc riêng, tab đang chọn tô đặc màu accent + chữ trắng, mô phỏng đúng thanh nav của website (`public/css/layout.css` `.main-nav`).
...
- **"Ảnh bìa sách hiển thị thế nào?"** — Trang Chi tiết sách dùng `resizeMode="contain"` (hiện đủ ảnh gốc, không crop, vì đây là nơi xem ảnh chính). Mọi chỗ khác (Trang chủ, Khám phá, Tất cả đánh giá) dùng `resizeMode="cover"` (lấp đầy ô hiển thị) để lưới ảnh đều, đẹp mắt hơn.
